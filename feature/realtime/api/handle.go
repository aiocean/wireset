...
// Handle manages WebSocket connections for a specific room.
// It handles room creation, user joining, and message processing.
// The function performs the following tasks:
// - Retrieves or creates a room based on the roomID
// - Adds the user to the room
// - Publishes a UserJoinedEvt event
// - Continuously reads and processes incoming messages
// - Handles disconnection and cleanup when the connection is closed
// Parameters:
// - conn: The WebSocket connection
// Errors:
// - Returns early if there are issues with room creation, user addition, or message processing
	...
	for {
		...
		if err := h.Registry.Handle(result[0].String(), conn, &result[1]); err != nil {
			if errors.Is(err, registry.ErrHandlerNotFound) {
				logger.Warn("handler not found", zap.String("topic", result[0].String()))
			} else {
